...
<InfinteLoader loadMore={fetchOrdersList} let:items let:totalCount>
	<h1 class="fw-bold mb-3 fs-5">
		{#if isEndUser}My{/if} Orders ({totalCount})
	</h1>
			...
			{#each items as order}
				...
				<div class="order-card">
					...
					{#if order.UserInfo?.Address}
						<p class="mt-3 m-0"><strong>Name:</strong> {order.UserInfo.Name}</p>
						<p class="m-0"><strong>Phone:</strong> {order.UserInfo.Phone}</p>
