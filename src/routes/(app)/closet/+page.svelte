...
{#if !$APP.User?.ActiveSubscription || (!Object.values($APP?.Orders || {}).length && !($APP.Articles && Object.keys($APP.Articles).length))}
	<ServiceCards />
{:else}
	<InfinteLoader loadMore={fetchArticles} let:items let:totalCount>
		...
		{#if !items.length}
			<div class="d-flex align-items-center flex-column mb-2">
				<img src="/imgs/empty-closet.png" alt="Empty closet" class="empty-state-image mb-3" />
